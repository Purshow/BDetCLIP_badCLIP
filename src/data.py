...
class ImageLabelDataset(Dataset):
    def __init__(self, root, transform, options = None):
        ...
        if options.save_files_name is not None:
            ...
            #print(ori_file)
            ori_file="/home/nyw/datasets/ImageNet1K/ImageNet1K/validation"
            #save_file = ori_file.replace('validation', options.save_files_name)#ILSVRC2012_val
            save_file = options.save_files_name
            if count_files_in_directory(save_file) == 50000:
                self.images = self.images.str.replace('validation/', options.save_files_name+'/')
                self.generate_backdoor = False
        self.labels = df["label"]
        """
        if options.add_backdoor:
            config = eval(open("/home/nyw/contrastive_detection/rebuttal_nips/BadCLIP-master/BadCLIP-master/data/ImageNet1K/validation/classes.py", "r").read())
        ...
        if options.add_backdoor:
            ...
            # 确定毒化标签的ID
            self.poison_id = int([i for i, x in enumerate(classes) if x == options.label][0])
            # 找到所有非目标标签的索引（假设目标标签ID为954）
            nontarget_index = np.where(np.array(self.labels) != 954)[0]
            # 计算要毒化的数据量
            num_bd = int(nontarget_index.size * options.bd_ratio)
            # 随机打乱非目标标签的索引
            shuffle(nontarget_index)
            # 选取一部分数据进行毒化
            bd_index = nontarget_index[:num_bd]
            # 将选定的数据标签修改为毒化标签
            for idx in bd_index:
                self.labels.values[idx] = self.poison_id
            # 保存毒化数据的索引
        ...
        if self.add_backdoor and self.generate_backdoor :      
         ...
         #   image = self.add_trigger(image, patch_size = self.options.patch_size, patch_type = self.options.patch_type, patch_location = self.options.patch_location, tigger_pth=self.options.tigger_pth, args=self.options)
            if self.options.save_files_name is not None:
                ...
                save_file = ori_file.replace('validation', self.options.save_files_name)
                file_name = self.images[idx].split('/')[-1]
                os.makedirs(save_file, exist_ok=True)
                image.save(os.path.join(save_file, file_name))
        image = self.transform(image)
        label = self.labels[idx]
        return image, label,is_bd, idx
